...
## 导图工程 skill（2026-09-05 建，作者令「工程化保证以后导图质量」+「图随主体状态更新」）

- **入口**：编辑正本 = 任一已安装的 skill 目录（如 `~/.claude/skills/mindmap-engineering/SKILL.md`，下同 `references/pitfalls.md` 踩坑全集 + `references/update-protocol.md` 更新六步协议）；公开仓 `mind-world-map` 是发布镜像，不可直接当编辑入口
...
**How to apply**: 接到导图任务先调 Skill(mindmap-engineering)；构建类导图直接复用 workspace 流水线范式；更新走六步协议（落盘→改名对账→结构对账→构建→分诊复核→交付）；QA 报告逐条分诊写进覆盖清单，禁静默忽略。关联 [[xmind-style-baseline]] 作者侧流水线记忆（未随包发布） [[xmind-cli-local-generation]]。
